...
![database_erd](https://github.com/boomup249/Graduation_Project/assets/112924147/7d766402-c474-4273-918d-af9e52036742)
 ### ① 유저 정보 테이블 : info, prefer, profile_data
   - info : 유저 기본 정보1(아이디, 이메일, 비밀번호, 생년월일, 성별)
   - prefer : 유저별 장르 정보(아이디, 선호장르, 비선호장르)
   - profile_data : 유저 기본 정보2(아이디, 닉네임, 프로필이미지, 상태메세지)
 ### ② 게시글 정보 테이블 : bbs, bbs_content, bbs_comment
   - bbs : 게시글 정보(글번호, 제목, 글작성자, 카테고리, 작성시간, 조회수, 댓글수)
   - bbs_content : 게시글 내용(분류번호, 글번호, 카테고리, 제목, 글작성자, 글내용)
   - bbs_comment : 게시글별 댓글 내용(댓글번호, 글번호, 댓글내용, 댓글작성자)
 ### ③ 크롤링 데이터 저장 테이블 : gamedata_*, gamedata_*_genre, release_info, crawling_time
   - gamedata_* : 게임 할인 정보(번호, 게임명, 정가, 할인가, 할인율, 게임소개, 대표이미지, 게임소개이미지, 판매주소)
   - gamedata_*_genre : 게임 장르 정보(번호, 게임명, 장르)
   - release_info : 출시예정 게임 정보(출시일자, 제목, 출시기종, 가격, 특이사항)
   - crawling_time : 크롤링 종료 시간

## 📲 데이터 크롤링
![loco_crawling_bot](https://github.com/boomup249/Graduation_Project/assets/112924147/5feb877a-0a3a-4c3d-b41d-8c1a276bbcac)
 ### ① 크롤링 대상 데이터
   - 할인을 진행중인 PC 게임, Console 게임과 출시 예정의 게임 정보를 크롤링해오는것이 목표
   - PC 게임은 Steam, Epicgames에서 크롤링, Console 게임은 Nintendo, Playstation에서 크롤링, 출시 예정 게임은 Danawa에서 크롤링
   - 할인 게임은 (제목, 정가, 할인가, 할인율, 게임설명, 대표이미지, 소개이미지, 판매처url) 총 8가지의 데이터를 각 사이트에서 크롤링
   - 출시 예정 게임은 (제목, 가격, 출시날짜, 플랫폼) 총 4가지의 데이터를 크롤링
 ### ② 동작 방식
   - 서버 컴퓨터에 파이썬이 설치돼있지않아도 크롤링을 진행할수있게 코드 전체를 실행파일로 만듦
   - 사용자가 프로그램을 종료하지않는 이상 백그라운드에 상주하며 매일 06시에 크롤링 진행 후 완료되면 할당된 메모리 해제한뒤 익일 06시까지 대기
   - 멀티프로세싱을 이용해 3개의 사이트에서 동시에 크롤링 진행(Steam, Danawa, Epicgames, Nintendo, Playstation 순서로 진행)
   - 각 사이트들은 정보를 동적으로 생성하기때문에 셀레니움 라이브러리를 이용하여 크롤링을 진행
   - 게임 하나의 데이터를 딕셔너리 형태로 리스트에 저장한 후 더이상 크롤링할 게임이 존재하지않으면 리스트의 모든 내용을 DB에 데이터 저장
